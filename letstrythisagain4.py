...
reset_button = pygame.Rect(600, 150, 140, 30)
button_des = pygame.Rect(600, 300, 140, 30)
button_end = pygame.Rect(600, 450, 140,30)
# Main loop
running = True
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if reset_button.collidepoint(event.pos):
                ...
                b = False
        if event.type == pygame.MOUSEBUTTONDOWN:
            if button_des.collidepoint(event.pos):
    ...
    pygame.draw.rect(screen, (0, 200, 20), button_end)
    end_text = font.render("End Sim", True, (200, 0, 140))
    screen.blit(end_text, (button_end.x + 5, button_end.y + 5))
    input_box_m1.update()
    input_box_m2.update()
    pygame.display.flip()
    clock.tick(60)
...
## notes: the desmos graph resets after clicking reset on the button and refreshing desmos.....
## (i made a boolean b to dictate this depending on if reset was clicked.....)--------------> try returning an empty set to the server if reset is pressed?