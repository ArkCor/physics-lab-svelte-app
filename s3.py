...
time_data = []
position1 = []  # x positions of the ball
position2 = []  # y positions of the ball
vel1 = []
...
def update_physics(ball):
    ...
    while not stop.is_set():
        ...
        position1.append(pos[0])  # Store x position
        position2.append(pos[1])  # Store y position
        ...
        data_to_send = [
            ['time', time_elapsed, 't'],
            ['x_pos1', pos[0], 'x_{1}'],  # Handle x position
            ['y_pos1', pos[1], 'y_{1}'],  # Handle y position
            ['vel1', vel1[-1] if vel1 else 0, 'v_{1}'],  # Handle velocity
            ['theta', theta, 'a_{1}']  # Handle angle (theta) make it a_{1} bc desmos doesnt like theta letter
        ]
